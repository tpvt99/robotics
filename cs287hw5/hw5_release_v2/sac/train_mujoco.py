...
def get_extra_param(extra_params, name, default):
    if extra_params is None or name not in extra_params:
        return default
...
def train_SAC(env_name, exp_name, seed, logdir, extra_params=None):
    ...
    algorithm_params = {
        ...
        'discount': 0.99,
        'learning_rate': 1e-3,
        'reparameterize': get_extra_param(extra_params, 'reparameterize', False),
        'tau': 0.01,
        'epoch_length': 1000,
        'n_epochs': 500,
...
def main():
    ...
    parser.add_argument('--reparameterize', default=False, action='store_true')
    parser.add_argument('--two_qf', default=False, action='store_true')
