...
class DiagonalGaussian(Distribution):
    ...
    def log_likelihood(self, xs, dist_info):
        ...
        sigma = tf.math.square(tf.linalg.diag(tf.squeeze(log_stds)))
        component2 = -1 / 2 * tf.math.log(tf.linalg.det(sigma))
        component3 = -1 / 2 * tf.linalg.matmul(tf.linalg.matmul(xs - means, tf.linalg.inv(sigma)),
                                               tf.transpose(xs - means))
